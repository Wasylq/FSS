...
Every release artifact also carries a SLSA v1 build attestation (`actions/attest-build-provenance` in the `release` job), verifiable with `gh attestation verify <file> --owner Wasylq`. `SHA256SUMS` proves a download arrived intact; the attestation is what proves it came from this pipeline, since the checksums are generated by the same job that builds the binaries.
