...
| `--organized` | bool | `true` | Set the organized flag on imported scenes |
...
| **SUBSTR** | A normalized title is a substring of the normalized filename (handles `"Studio - Title"` naming). When multiple titles match, the longest (most specific) wins |
...
3. **Resolution tags** (when `--resolution-tags` is set):
...
2. A JSON changelog entry is appended to `fss-stashbox-changelog.json` in the output directory, recording:
