...
func (s *Scraper) run(ctx context.Context, studioURL string, opts scraper.ListOpts, out chan<- scraper.SceneResult) {
	...
	for page := 1; ; page++ {
		...
		scraper.Debugf(1, "privateclassics: fetching page %d", page)
		...
		if !sentTotal {
			...
			scraper.Debugf(1, "privateclassics: ~%d total scenes (estimated)", total)
			if total > 0 {
				select {
		...
		for _, item := range items {
			if opts.KnownIDs[item.id] {
				scraper.Debugf(1, "privateclassics: hit known ID %s, stopping early", item.id)
				select {
