...
func TestMatchesURL(t *testing.T) {
	...
	cases := []struct {
		...
		{"https://www.lethalhardcore.com", true},
		{"https://lethalhardcore.com/en/videos", true},
		{"https://www.lethalhardcore.com/en/video/lethalhardcore/Some-Scene/12345", true},
...
func TestToScene(t *testing.T) {
	...
	scene := gammautil.ToScene(config, "https://www.lethalhardcore.com", hit, now)

	if scene.ID != "100001" {
		t.Errorf("ID = %q", scene.ID)
	}
	if scene.SiteID != "lethalhardcore" {
		t.Errorf("SiteID = %q", scene.SiteID)
	}
	if scene.URL != "https://www.lethalhardcore.com/en/video/lethalhardcore/Test-Scene/100001" {
		t.Errorf("URL = %q", scene.URL)
	}
	if scene.Studio != "Lethal Hardcore" {
...
func TestListScenes(t *testing.T) {
	hits := []gammautil.AlgoliaHit{
		{
			...
			Categories:  []gammautil.Category{{Name: "Hardcore"}},
	...
	cfg := gammautil.SiteConfig{
		SiteID: "lethalhardcore", SiteBase: ts.URL, StudioName: "Lethal Hardcore", SiteName: "lethalhardcore",
	}
