// Package aziani scrapes the Aziani network sites that run on the NATS CMS
// (azianistudios.com). The four sub-sites (aziani.com, 2poles1hole.com,
// creampiled.com, popuporgies.com) share a single content pool of ~3300
// scenes; each site has its own cms-area-id but the API returns the full
// catalogue regardless of which site's UUID is used.
//
// The NATS CMS pattern is identical to the marsmedia package (My Gay Cash
// NATS CMS) — only the API host, scene URL format, and studio name differ.
package aziani
...
const (
	natsAPIBase = "https://azianistudios.com/tour_api.php"
	studioName  = "Aziani"
)
...
func (s *Scraper) fetchSets(ctx context.Context, blockID string) (*setsResponse, error) {
	u := s.apiBase + "/content/sets?cms_block_id=" + blockID
	body, err := s.fetchAPI(ctx, u)
	if err != nil {
		return nil, err
	}
...
func (s *Scraper) run(ctx context.Context, studioURL string, opts scraper.ListOpts, out chan<- scraper.SceneResult) {
	defer close(out)
	scraper.Debugf(1, "aziani/%s: discovering set_list block via NATS CMS", s.cfg.ID)
	...
	scraper.Debugf(1, "aziani/%s: set_list block_id=%s", s.cfg.ID, blockID)

	servers, err := s.fetchServers(ctx)
	if err != nil {
		scraper.Debugf(1, "aziani/%s: servers fetch failed (%v) — thumbnails omitted", s.cfg.ID, err)
		servers = nil
	} else {
		scraper.Debugf(1, "aziani/%s: %d CDN server(s)", s.cfg.ID, len(servers))
	}
	...
	scraper.Debugf(1, "aziani/%s: %d total scenes", s.cfg.ID, total)
	if total > 0 {
		select {
	...
	for _, entry := range sets.Sets {
		if opts.KnownIDs[entry.CMSSetID] {
			scraper.Debugf(1, "aziani/%s: hit known ID %s, stopping early", s.cfg.ID, entry.CMSSetID)
			select {
