...
func TestListScenes(t *testing.T) {
	// The scraper follows each <loc> verbatim, so the fixture's live host must
	// be rewritten to this server. Overriding siteBase only redirects the
	// sitemap request; the detail fetches would still go to kristenbjorn.com.
	srv := testutil.SitemapServer(t, "https://www.kristenbjorn.com",
