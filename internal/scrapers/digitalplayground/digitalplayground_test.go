...
func TestMatchesURL(t *testing.T) {
	...
	cases := []struct {
		...
		{"https://www.digitalplayground.com", true},
		{"https://digitalplayground.com/pornstar/123/some-star", true},
		{"https://www.digitalplayground.com/category/79/milf", true},
...
func TestListScenes(t *testing.T) {
	releases := []ayloutil.Release{
		{
			ID: 11001, Type: "scene", Title: "DP Scene",
	...
	cfg := ayloutil.SiteConfig{SiteID: "digitalplayground", SiteBase: ts.URL, StudioName: "Digital Playground"}
	...
	if len(results) != 1 || results[0].Title != "DP Scene" {
...
func TestListScenesKnownIDs(t *testing.T) {
	releases := []ayloutil.Release{
		{ID: 12001, Type: "scene", Title: "New", DateReleased: "2026-02-01T12:00:00+00:00",
			RawImages: json.RawMessage(`[]`), RawVideos: json.RawMessage(`[]`)},
		{ID: 12002, Type: "scene", Title: "Known", DateReleased: "2026-01-01T12:00:00+00:00",
			RawImages: json.RawMessage(`[]`), RawVideos: json.RawMessage(`[]`)},
	}
	...
	ch, err := s.ListScenes(context.Background(), ts.URL, scraper.ListOpts{
		KnownIDs: map[string]bool{"12002": true},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(results) != 1 || results[0].ID != "12001" {
		t.Errorf("got scenes %v, want [12001]", results)
	}
}
