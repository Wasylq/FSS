...
func TestMatchesURL(t *testing.T) {
	...
	cases := []struct {
		...
		{"https://www.girlfriendsfilms.com", true},
		{"https://girlfriendsfilms.com/en/videos", true},
		{"https://www.girlfriendsfilms.com/en/video/girlfriendsfilms/Some-Scene/12345", true},
...
func TestToScene(t *testing.T) {
	...
	scene := gammautil.ToScene(config, "https://www.girlfriendsfilms.com", hit, now)

	if scene.ID != "100001" {
		t.Errorf("ID = %q", scene.ID)
	}
	if scene.SiteID != "girlfriendsfilms" {
		t.Errorf("SiteID = %q", scene.SiteID)
	}
	if scene.URL != "https://www.girlfriendsfilms.com/en/video/girlfriendsfilms/Test-Scene/100001" {
		t.Errorf("URL = %q", scene.URL)
	}
	if scene.Studio != "Girlfriends Films" {
...
func TestListScenes(t *testing.T) {
	hits := []gammautil.AlgoliaHit{
		{
			...
			Categories:  []gammautil.Category{{Name: "Lesbian"}},
	...
	cfg := gammautil.SiteConfig{
		SiteID: "girlfriendsfilms", SiteBase: ts.URL, StudioName: "Girlfriends Films", SiteName: "girlfriendsfilms",
	}
