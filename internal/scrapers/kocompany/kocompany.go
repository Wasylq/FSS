...
func (s *Scraper) run(ctx context.Context, studioURL string, opts scraper.ListOpts, out chan<- scraper.SceneResult) {
	...
	for page := 1; ; page++ {
		...
		scraper.Debugf(1, "kocompany/%s: fetching page %d", s.cfg.ID, page)
		...
		if !sentTotal {
			...
			scraper.Debugf(1, "kocompany/%s: ~%d total (estimated)", s.cfg.ID, total)
			if total > 0 {
				select {
		...
		for _, item := range items {
			if opts.KnownIDs[item.id] {
				scraper.Debugf(1, "kocompany/%s: hit known ID %s, stopping early", s.cfg.ID, item.id)
				select {
