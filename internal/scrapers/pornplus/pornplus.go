...
func (s *Scraper) run(ctx context.Context, studioURL string, opts scraper.ListOpts, out chan<- scraper.SceneResult) {
	...
	if m := modelRe.FindStringSubmatch(studioURL); m != nil {
		...
		// The actor endpoint returns releases in no date order at all and
		// ignores `sort=latest` (live-verified: page 1 of a 73-release actor
		// runs 2025, 2024, 2023, 2022, 2024, …). A KnownIDs early-stop there
		// would halt on the first stored scene and never reach the newer ones
		// behind it, so the hint is dropped for this mode. The main listing
		// below is genuinely newest-first and keeps it.
		opts.KnownIDs = nil
	} else {
