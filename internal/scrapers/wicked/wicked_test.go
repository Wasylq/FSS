...
func TestMatchesURL(t *testing.T) {
	...
	cases := []struct {
		...
		{"https://www.wicked.com", true},
		{"https://wicked.com/en/videos", true},
		{"https://www.wicked.com/en/video/wicked/Some-Scene/12345", true},
...
func TestToScene(t *testing.T) {
	...
	scene := gammautil.ToScene(config, "https://www.wicked.com", hit, now)

	if scene.ID != "100001" {
		t.Errorf("ID = %q", scene.ID)
	}
	if scene.SiteID != "wicked" {
		t.Errorf("SiteID = %q", scene.SiteID)
	}
	if scene.URL != "https://www.wicked.com/en/video/wicked/Test-Scene/100001" {
		t.Errorf("URL = %q", scene.URL)
	}
	if scene.Studio != "Wicked" {
...
func TestListScenes(t *testing.T) {
	hits := []gammautil.AlgoliaHit{
		{
			...
			Categories:  []gammautil.Category{{Name: "Feature"}},
	...
	cfg := gammautil.SiteConfig{
		SiteID: "wicked", SiteBase: ts.URL, StudioName: "Wicked", SiteName: "wicked",
	}
