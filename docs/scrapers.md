...
| Virtual Taboo CMS (3 sites) | `{domain}/videos`, `{domain}/model/{slug}` | Custom | No | Table-driven `virtualtaboo` package: HTML listing with `video-card__item` cards (10/page), JSON-LD VideoObject on detail pages for description/date/duration, `/model/{slug}` model pages, tag links from detail page. Sites: [Dark Room VR](https://darkroomvr.com), [OnlyTarts](https://onlytarts.com), [Virtual Taboo](https://virtualtaboo.com) |
