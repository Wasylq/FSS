...
| Glamose / UTG Networks (10 sites) | `{domain}/updates/videos`, `{domain}/models/{slug}` | UTG Networks CMS | No | Table-driven `glamose` package using `utgutil`. Two template variants: new (Tailwind, `<article>` tags, 200/page) and legacy (Bootstrap 3, year-based or page-based pagination). CDN slug IDs from `assets.utgnetworks.com`, model page support on new template, ~2.8k scenes. Sites: Hayley's Secrets (hayleyssecrets.com), More Than Nylons (morethannylons.com), Skin Tight Glamour (skintightglamour.com), UK Tickling (uktickling.com), Worship Jasmine (worshipjasmine.com), All Brook Wright (allbrookwright.com), Beth Morgan Official (bethmorganofficial.com), Sophia's Sexy Legwear (sophiassexylegwear.com), BreathTakers (breath-takers.com), Girlfolio (girlfolio.com) |
